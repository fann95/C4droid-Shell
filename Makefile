...
install-autotools:
	if ! [ -f $(iPREFIX)/usr/bin/automake ]; then cd $(iPREFIX) && tar -mzxf $(PWD)/App/Autotools/usr.tar.gz;fi;
...
install-cmake: install-ssl
	...
	if ! [ -d $(sPREFIX)/share/cmake-3.0 ]; then cd /mnt/sdcard && tar -mzxf $(PWD)/App/cmake/C4droid_EXT.tar.gz;fi;